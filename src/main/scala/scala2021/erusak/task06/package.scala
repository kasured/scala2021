...
package task06 {
  ...
  import scala2021.erusak.task06.SexType.SexType
  ...
  object Validate {
    ...
    private[task06] def nameValidation(input: Contract): Either[String, Contract] = input.name.value match {
    ...
    private[task06] def ageValidation(input: Contract): Either[String, Contract] = input.age.value match {
      case age if age < 0 => Left("Age cannot be negative")
      case age if age > 100 => Left("Age cannot be greater than 100")
      case _ => Right(input)
    }

    private[task06] def emailValidation(input: Contract): Either[String, Contract] = input.email.value match {
      case email if email.isEmpty => Right(input)
      case email if !email.matches(".+\\@.+\\..+") => Left("Email is not valid")
      case _ => Right(input)
    }

    private[task06] def sexValidation(input: Contract): Either[String, Contract] = input.sex match {

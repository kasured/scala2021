...
object ResourceApi extends StrictLogging {
   ...
   */
  private[task07] def withResource[T, R](initResource: => T, releaseResource: T => Unit = (_: T) => ())(op: T => R): Try[R] = {
    val tryResource = Try {
      initResource
    }
  ...
  private[task07] final case class ConnectionSuccess(port: Int) extends Connection {
  ...
  private[task07] final case class ConnectionWithExceptionOnInit(port: Int) extends Connection {
  ...
  private[task07] final case class ConnectionWithExceptionOnRun(port: Int) extends Connection {
  ...
  private[task07] final case class ConnectionWithExceptionOnClose(port: Int) extends Connection {
